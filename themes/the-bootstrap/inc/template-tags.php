...
function the_bootstrap_navbar_searchform( $echo = true ) {
	//CUSTOM CODE
	wp_enqueue_script("selection_scripts", get_template_directory_uri() . "/js/bootstrap-select.min.js" );
	wp_enqueue_script("custom_scripts", get_template_directory_uri() . "/js/custom_theme.js" );
	wp_enqueue_style("selection_styles", get_template_directory_uri() . "/css/bootstrap-select.min.css");
	$searchform = ' <style type="text/css">
			.custom_select_btn {
			...
			.search-query {
			  width: 152px !important;
                	...
                	<form id="searchform" class="navbar-search" method="get" action="' . esc_url( home_url( '/' ) ) . '">
						<label for="s" class="assistive-text hidden">' . __( 'Quick Search', 'the-bootstrap' ) . '</label>
						<input type="hidden" name="theme_action" value="theme_search" />
						<input type="search" class="search-query" name="s" id="s" placeholder="' . esc_attr__( 'Quick Search', 'the-bootstrap' ) . '" />
					        <select name="theme_post_type" class="selectpicker show-tick" data-style="custom_select_btn">
							<option value="rental">' . __('Rent', 'epl') . '</option>
							<option value="property">' . __('Buy', 'epl') . '</option>
							<option value="business">' . __('Business', 'epl') . '</option>
					        </select>
					</form>';
